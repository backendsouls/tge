...
func TestNewLevel(t *testing.T) {
	if _, err := progression.NewLevel(1, "  ", 0, 0); err != progression.ErrInvalidLevelName {
		t.Errorf("blank name: got %v", err)
	}
	if _, err := progression.NewLevel(0, "First", 0, 0); err == nil {
		t.Error("expected error for non-positive number")
	}
	if _, err := progression.NewLevel(1, "First", -1, 0); err != progression.ErrInvalidLevelPoints {
		t.Errorf("negative points: got %v", err)
	}
	l, err := progression.NewLevel(1, " First Level ", 100, 20)
...
func TestRealm_AddLevel(t *testing.T) {
	r, err := progression.NewRealm(progression.RealmConfig{Name: "Qi Condensation"})
	if err != nil {
	...
	if err := r.AddLevel(1, "dup", 0, 0); !errors.Is(err, progression.ErrLevelNumberExists) {
		t.Fatalf("duplicate number: got %v", err)
	}
}

func TestRealm_AddLevel_MaxLevels(t *testing.T) {
	r, err := progression.NewRealm(progression.RealmConfig{Name: "Qi Condensation", MaxLevels: 2})
	if err != nil {
		t.Fatalf("new realm: %v", err)
	}
	if err := r.AddLevel(1, "First Level", 0, 0); err != nil {
	...
	if err := r.AddLevel(3, "Third Level", 0, 0); !errors.Is(err, progression.ErrLevelNumberExceedsMax) {
		t.Fatalf("over cap: got %v, want ErrLevelNumberExceedsMax", err)
	}
	// MaxLevels 0 means unlimited.
	unlimited, _ := progression.NewRealm(progression.RealmConfig{Name: "Boundless"})
...
func TestRealm_MaxLevelsFor(t *testing.T) {
	...
	// ...but not beyond it.
	if err := r.AddLevel(14, "Fourteenth Level", 0, 0); !errors.Is(err, progression.ErrLevelNumberExceedsMax) {
		t.Fatalf("level 14 over main cap: got %v", err)
	}

	// When the main cap is unset, the main character inherits the normal cap.
	r2, _ := progression.NewRealm(progression.RealmConfig{Name: "Foundation", MaxLevels: 9})
