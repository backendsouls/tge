...
func NewSkill(name, description, grade string) (Skill, error) {
	name = strings.TrimSpace(name)
	if name == "" {
		return Skill{}, ErrInvalidSkillName
	}
	return Skill{Name: name, Description: strings.TrimSpace(description), Grade: strings.TrimSpace(grade)}, nil
}
