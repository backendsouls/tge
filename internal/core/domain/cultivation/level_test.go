...
func TestNewLevel(t *testing.T) {
	if _, err := cultivation.NewLevel(1, "  ", 0); err != cultivation.ErrInvalidLevelName {
		t.Errorf("blank name: got %v", err)
	}
	if _, err := cultivation.NewLevel(0, "First", 0); err == nil {
		t.Error("expected error for non-positive number")
	}
	if _, err := cultivation.NewLevel(1, "First", -1); err != cultivation.ErrInvalidLevelPoints {
		t.Errorf("negative points: got %v", err)
	}
	l, err := cultivation.NewLevel(1, " First Level ", 100)
	if err != nil || l.Name != "First Level" || l.Number != 1 {
		t.Fatalf("level: %+v err=%v", l, err)
	}
	if l.BreakthroughPoints != 100 {
		t.Errorf("points = %d, want 100", l.BreakthroughPoints)
	}
}
...
func TestRealm_AddLevel(t *testing.T) {
	...
	if err := r.AddLevel(3, "Third Level", 0); err != nil {
		t.Fatalf("add level 3: %v", err)
	}
	if err := r.AddLevel(1, "First Level", 0); err != nil {
		t.Fatalf("add level 1: %v", err)
	}
	if len(r.Levels) != 2 || r.Levels[0].Number != 1 || r.Levels[1].Number != 3 {
		t.Fatalf("levels not sorted: %+v", r.Levels)
	}
	if err := r.AddLevel(1, "dup", 0); !errors.Is(err, cultivation.ErrLevelNumberExists) {
...
func TestRealm_AddLevel_MaxLevels(t *testing.T) {
	...
	if err := r.AddLevel(2, "Second Level", 0); err != nil {
		t.Fatalf("add level 2: %v", err)
	}
	if err := r.AddLevel(3, "Third Level", 0); !errors.Is(err, cultivation.ErrLevelNumberExceedsMax) {
	...
	if err := unlimited.AddLevel(999, "Far Level", 0); err != nil {
...
func TestRealm_MaxLevelsFor(t *testing.T) {
	...
	if err := r.AddLevel(13, "Thirteenth Level", 0); err != nil {
		t.Fatalf("level 13 within main cap rejected: %v", err)
	}
	// ...but not beyond it.
	if err := r.AddLevel(14, "Fourteenth Level", 0); !errors.Is(err, cultivation.ErrLevelNumberExceedsMax) {
